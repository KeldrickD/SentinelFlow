...
function computeAction(deviationBps, config) {
  if (deviationBps >= config.pauseBpsThreshold)
    return "PAUSE";
  if (deviationBps >= config.deviationBpsThreshold)
    return "SET_RISK_MODE";
  return "NO_ACTION";
}
var onHttpTrigger = (runtime2, payload) => {
  ...
  runtime2.log(`SentinelFlow: deviationBps=${deviationBps} reason=${reason} actionType=${actionType} exceeded=${exceeded}`);
  return JSON.stringify({
    actionType,
    setRiskMode,
    exceeded,
    deviationBps,
    reason: reason || (exceeded ? "threshold exceeded" : "within band")
  });
};
