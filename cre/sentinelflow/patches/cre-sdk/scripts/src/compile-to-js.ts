...
export const main = async (tsFilePath?: string, outputFilePath?: string) => {
	...
	// Ensure the output directory exists (ignore EEXIST on Windows/Bun)
	try {
	...
	try {
		...
		// Bundle into the final file (overwrite) — use spawn so paths with spaces work on Windows
		await new Promise<void>((resolve, reject) => {
			const proc = spawn(process.execPath, ['build', builtFile, '--bundle', '--outfile', resolvedOutput], {
				stdio: 'inherit',
				cwd: path.dirname(resolvedInput),
			})
			proc.on('close', (code) => (code === 0 ? resolve() : reject(new Error(`bun build exited ${code}`))))
