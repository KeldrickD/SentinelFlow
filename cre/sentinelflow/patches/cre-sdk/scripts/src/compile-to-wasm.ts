...
export const main = async (inputFile?: string, outputFile?: string) => {
	...
	// Ensure output directory exists (ignore EEXIST on Windows/Bun)
	try {
	...
	// Run compilation — use spawn so paths with spaces work on Windows
	...
	await new Promise<void>((resolve, reject) => {
		const proc = spawn(process.execPath, ['--bun', compilerPath, resolvedInput, resolvedOutput], {
			stdio: 'inherit',
		})
		proc.on('close', (code) => (code === 0 ? resolve() : reject(new Error(`compile-workflow exited ${code}`))))
